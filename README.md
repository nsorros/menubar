...
| **Cloud spend** | OpenRouter credit balance + spend, and AWS daily/monthly spend against budget | [`cloud-spend/`](cloud-spend/) |
...
- **AWS CLI** — only for Cloud spend's AWS half (`brew install awscli`).
...
# Cloud spend (needs an OpenRouter key + AWS billing key in the keychain — see its README)
ln -sf "$PWD/cloud-spend/cloud-spend.10m.py" "$PLUGINS/"
```
